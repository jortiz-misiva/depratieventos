...
class Mec_Taxonomy_Search_Bar{
    ...
    public function display($skin_class,$taxonomy){
        ...
        ?>
        <!-- begin searchbar -->
        <div id="mec-search-box" class="mec-advanced-speaker mec-search-box mec-totalcal-box">
                ...
                <div class="row">
                    <div class="col-md-6">
                            <?php if(true === $skin_class->search): ?>
                                <input type="text" class="mec-search-field" name="s2" placeholder="<?php esc_attr_e('Search','mec-advanced-speaker') ?>" value="<?php echo isset($_REQUEST['s2']) ? esc_attr($_REQUEST['s2']) : ''; ?>" />
                            <?php endif; ?>
                            ...
                            <select name="orderby" class="mec-search-field">
                                <?php
                                    $orderby = apply_filters(
                                        ...
                                        array(
                                            '' => __('Sort by','mec-advanced-speaker'),
                                            'name' => __('Name','mec-advanced-speaker'),
                                            'added_date' => __('Added Date','mec-advanced-speaker'),
                                            // 'ongoing_events' => __('Ongoing Events','mec-advanced-speaker'),
                                            'all_events' => __('All Events','mec-advanced-speaker'),
                                        )
                                    );
                            ...
                            <select name="order" class="mec-search-field second">
                                <option value=""><?php esc_html_e('Sort','mec-advanced-speaker') ?></option>
                                <option value="DESC" <?php selected($selected,'DESC') ?>><?php esc_html_e('DESC','mec-advanced-speaker') ?></option>
                                <option value="ASC" <?php selected($selected,'ASC') ?>><?php esc_html_e('ASC','mec-advanced-speaker') ?></option>
                            </select>
                            <?php endif; ?>
                            <button class="button"><?php _e('Filter','mec-advanced-speaker') ?></button>
