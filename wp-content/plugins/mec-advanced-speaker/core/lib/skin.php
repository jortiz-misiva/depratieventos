 ...
 */
class MEC_Advanced_Speaker_Lib_Skin {

	public $order_by = 'id';
	public $order = 'ASC';
	public $section = 'speaker';
	public $limit = 0;
	public $offset = 0;
	public $count = 3;
	public $data;
	...
	public function __construct() {
		...
		if (isset($options['settings']) && $options['settings']['advanced_speaker']) {
			$this->settings = $options['settings']['advanced_speaker'];
	...
	public function initialize($atts = array()) {
		...
		$this->limit = isset($atts['limit']) ? $atts['limit'] : MEC_ADVANCED_SPEAKER_ROWS_LIMIT;
		...
		if( file_exists( $custom_tpl_path ) ) {
			...
			$this->tpl_path = MEC_ADVANCED_SPEAKER_DIR . 'core' . DS . 'skins' . DS . $this->style . DS . 'tpl.php';
		}
		...
		if( file_exists( $custom_render_path ) ) {
			...
			$this->render_path = MEC_ADVANCED_SPEAKER_DIR . 'core' . DS . 'skins' . DS . $this->style . DS . 'render.php';
		}
	...
	public function count_of_ongoing($id) {
		...
		$atts['from_advanced_speaker_addon'] = 'count_ongoing';
		$SKO->initialize($atts);
		$SKO->skin = 'list';
		$SKO->style = 'fluent';
	...
	public function single_page_url($id, $website = null) {
		...
		if( is_null( $website ) ){
			...
			$website = get_term_meta( $id, 'website', true );
		}
