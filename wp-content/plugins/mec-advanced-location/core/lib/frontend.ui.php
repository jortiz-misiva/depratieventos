 ...
 */
class MEC_Advanced_Location_Lib_Frontend_UI {
	...
	private static function init($class) {
		...
		self::$factory->shortcode('advanced-location-single-public', array($class, 'single_public_content'));
	...
	public static function single_public_content($atts = null) {
		...
		if ((is_admin() && $edit == 'edit' && $post != null) || $rest_route != null) {
			...
			if (empty($section) || empty($id)) {
				...
				return __('<h3>Modern Event Calendar Advanced Location Single Page</h3>', 'mec-advanced-location');
			}
			...
			switch ($section) {
			case 'location':
				echo \MEC_Advanced_Location\Core\LocationWidget\MEC_Advanced_Location_LocationWidget_Frontend::single_location_content($atts);
				break;
			}
		}
	...
	public static function change_dates($dates, $start, $end, $skin) {
		...
		if (!isset($skin->atts['from_advanced_location_addon'])) {
			return $dates;
		}
		...
		if ($skin->atts['from_advanced_location_addon'] === 'count_ongoing') {
